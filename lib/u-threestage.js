...
let ThreeStage = {
	...
	create : async (prop = {}) => {
		...
		if(typeof prop.draggable === 'boolean')
			stage.params.stage.draggable = prop.draggable;
		if(typeof prop.drag_button === 'number')
			stage.params.stage.drag_button = prop.drag_button;
		if(typeof prop.pan_speed === 'number')
			stage.params.stage.pan_speed = prop.pan_speed;
		else if(typeof prop.drag_sensitivity === 'number')
			stage.params.stage.pan_speed = prop.drag_sensitivity;
		...
		stage.init = async () => {
			...
			stage.stop = () => {
				...
				stage.mouse = {};
				if(typeof stage.params.stage.pan_speed !== 'number')
					stage.params.stage.pan_speed = 0.03;
			stage.updateMouseFromEvent = (e) => {
				const rect = stage.renderer.view.getBoundingClientRect();
				const nx = (e.clientX - rect.left) / rect.width;
				const ny = (e.clientY - rect.top) / rect.height;

				// Keep true NDC for raycasting.
				stage.mouse.x = nx * 2 - 1;
				stage.mouse.y = -(ny * 2) + 1;
			};
			...
			stage.renderer.view.addEventListener('mousedown', (e) => {
				...
				stage.renderer.view.addEventListener('mousemove', (e) => {
					// Update normalized mouse coordinates for raycasting and drag coordinates for panning.
					...
					if(stage.state['mouse_button_'+stage.params.stage.drag_button] &&
						...
						stage.state.mouse.x_diff = (e.clientX-stage.state.mouse.x_down) * stage.params.stage.pan_speed;
						stage.state.mouse.y_diff = (e.clientY-stage.state.mouse.y_down) * stage.params.stage.pan_speed;
				...
				window.addEventListener('blur', () => {
					for(const key in stage.state) {
						if(key.indexOf('mouse_button_') === 0)
							stage.state[key] = false;
					}
					if(stage.current_drag_object) {
						stage.current_drag_object.is_dragging = false;
						stage.current_drag_object.is_drag_moving = false;
					}
		...
		// Initialize materials and shapes
			stage.mat = {
				cursor: new THREE.MeshStandardMaterial({ 
					color: 0x156289, 
					emissive: 0x072534, 
