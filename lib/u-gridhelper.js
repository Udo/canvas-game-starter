(function (root, factory) {
	if (typeof exports === 'object' && typeof module !== 'undefined') {
		...
		root.GridMap3D = factory();
	}
}(typeof self !== 'undefined' ? self : this, function () {

// todo: this needs to be refactored, badly

// Assume bindThis is available globally or imported
if (typeof bindThis === 'undefined') {
	var bindThis = function(self, source, destination) {
		if(!destination)
			destination = {};
		if(!source) {
			...
			for(var prop in source) if(source.hasOwnProperty(prop)) {
				destination[prop] = source[prop].bind ? source[prop].bind(self) : source[prop];
			}
		}
		return(destination);
	};
...
var GridMap3D = bindThis({
  ...
  init : function(grid, stage) {
    ...
    this.grid = grid;
    this.stage = stage;
    this.THREE = (stage && stage.properties && stage.properties.THREE)
      || (typeof THREE !== 'undefined' ? THREE : null);
    if (!this.THREE) {
      throw new Error('GridMap3D.init: THREE not available. Pass stage.properties.THREE.');
    }
    this.createCursor();
  ...
  createGroundPlane : function() {
    this.groundPlane = new this.THREE.Mesh(
      new this.THREE.PlaneGeometry(1000, 1000, 1),
  ...
  createCursor : function() {
    this.cursor = this.stage.wireframe(new this.THREE.Mesh( this.stage.shapes.tile, this.stage.mat.cursor ));
